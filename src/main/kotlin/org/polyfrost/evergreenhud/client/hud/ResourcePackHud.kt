...
class ResourcePackHud : Hud(
    ...
    override fun update(): Boolean {
        ...
        val vanillaId = BuiltInPackSource.VANILLA_ID
        //?} else {
        /*val repository = ResourcePackRepository.client()
        val vanillaId = ClientPackSource.DEFAULT_PACK_ID
        *///?}

        val selected = repository.selectedPacks
        syncCaches(selected.map { it.id })

        val candidates = selected.filter {
            it.id != vanillaId && !isModProvided(it) && hasTextures(it)
        }
        val pack = (if (ignoreOverlay) candidates.firstOrNull() else candidates.lastOrNull())
            ?: repository.getPack(vanillaId)
